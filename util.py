...
def calculate_enc(seq):
    ...
    return codonbias.scores.EffectiveNumberOfCodons().get_score(seq)
...
def get_anti_codon(codon: str) -> str:
    ...
    for n in range(len(codon) - 1, -1, -1):
        anti_codon += get_nucleotide_watson_crick(codon[n])
    return anti_codon
...
def calculate_tai(seq: str) -> float:
    ...
    for codon in all_codons:
        ...
        for trna, copy_number in trna_dict.items():
            ...
            if get_anti_codon(trna) not in codon_friends:
                continue
